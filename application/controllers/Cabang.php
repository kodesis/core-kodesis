...
class Cabang extends CI_Controller
{
    public function __construct()
    {
        ...
        $this->load->model('cabang_m', 'cabang');
        $this->load->database();
    }
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $cat) {
            ...
            $row[] = '<center> <div class="list-icons d-inline-flex">
                <a title="Update User" href="' . base_url('cabang/update/' . $cat->uid) . '" class="btn btn-warning"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit">
                                                        <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                                                        <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                                                    </svg></a>
                                                <a title="Delete User" onclick="onDelete(' . $cat->uid . ')" class="btn btn-danger"><svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1">
    ...
    public function update($id)
    {
        ...
        if ($this->session->userdata('isLogin') == FALSE) {
            ...
            $data['detail'] = $this->cabang->get_detail_id($id);
    ...
    public function proses_tambah_cabang()
    {
        ...
        $this->cb->insert('t_cabang', $data_insert);
        redirect('cabang');
    }

    public function proses_update_cabang()
    {
        ...
        $this->cb->where('uid', $this->input->post('id_cabang')); // Ensure to specify the record to update
        $this->cb->update('t_cabang', $data_insert);
        redirect('cabang');
    }
    ...
    public function delete()
    {
        ...
        $this->cabang->delete(array('uid' => $id));
        echo json_encode(array("status" => TRUE));
    }

    public function getNextTabunganNumber()
    {
        $this->load->model('cabang_m'); // Load your model
        $jenis_tabungan = $this->input->post('jenis_tabungan');

        if ($jenis_tabungan) {
            // Get the highest `no_urut` across all records
            $lastNoUrut = $this->cabang_m->getLastNoUrut();
            $lastNumber = $this->cabang_m->getLastNumber($jenis_tabungan);
