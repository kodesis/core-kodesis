...
class Financial extends CI_Controller
{
	...
	public function invoice()
	{
		...
		$coa_13010_arr = $this->m_coa->getCoaByCode('13010');

		// Ambil data COA kedua
		$coa_25021 = $this->m_coa->getCoaByCode('25021');

		// Gabungkan kedua hasil ke dalam satu array baru
		$merged_coa_arr = array_merge($coa_410_arr, $coa_13020_arr, $coa_13010_arr, $coa_25021);
	...
	public function export_excel_invoice()
	{
		...
		if (empty($tanggal_dari) || empty($tanggal_sampai)) {
			$this->session->set_flashdata('error', 'Tanggal dari dan tanggal sampai harus diisi!');
			redirect('financial/invoice');
		}

		// Get data invoice
		$invoices = $this->m_invoice->get_invoice_report($id_customer, $tanggal_dari, $tanggal_sampai);
		...
		$objPHPExcel->getProperties()
			->setCreator("Kodesis")
			->setLastModifiedBy("Kodesis")
			->setTitle("Report Invoice")
			->setSubject("Report Invoice")
			->setDescription("Report Invoice periode " . date('d/m/Y', strtotime($tanggal_dari)) . " - " . date('d/m/Y', strtotime($tanggal_sampai)));
		...
		// Set title
		$sheet->setCellValue('A1', 'REPORT INVOICE');
		$sheet->mergeCells('A1:L1');
		$sheet->getStyle('A1')->getFont()->setBold(true)->setSize(16);
		$sheet->getStyle('A1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

		// Set periode
		$customer_name = ($id_customer == 'all') ? 'Semua Customer' : $this->M_Customer->get_customer_name($id_customer);
		$sheet->setCellValue('A2', 'Customer: ' . $customer_name);
		$sheet->mergeCells('A2:L2');
		$sheet->setCellValue('A3', 'Periode: ' . date('d/m/Y', strtotime($tanggal_dari)) . ' - ' . date('d/m/Y', strtotime($tanggal_sampai)));
		...
		foreach ($headers as $header) {
			...
			$sheet->getStyle($col . $header_row)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
			$col++;
		}
		...
		foreach ($invoices as $invoice) {
			...
			$sheet->setCellValue('B' . $row, $invoice->slug);
			$sheet->setCellValue('C' . $row, date('d/m/Y', strtotime($invoice->tanggal_invoice)));
			$sheet->setCellValue('D' . $row, $invoice->nama_customer);
			$sheet->setCellValue('E' . $row, $invoice->subtotal);
			$sheet->setCellValue('F' . $row, $invoice->besaran_diskon);
			$sheet->setCellValue('G' . $row, $invoice->besaran_ppn);
			$sheet->setCellValue('H' . $row, $invoice->besaran_pph);
			$sheet->setCellValue('I' . $row, $invoice->total_denganpph);
			$sheet->setCellValue('J' . $row, ($invoice->status_bayar == 1) ? 'Lunas' : 'Belum Lunas');
			$sheet->setCellValue('K' . $row, !empty($invoice->tanggal_bayar) ? date('d/m/Y', strtotime($invoice->tanggal_bayar)) : '-');
			$sheet->setCellValue('L' . $row, $invoice->keterangan);
			$sheet->setCellValue('M' . $row, ($invoice->status_void == '1') ? 'Sudah di-void tanggal ' . $invoice->tanggal_void : '-');
			...
			$total_diskon += $invoice->besaran_diskon;
			$total_ppn += $invoice->besaran_ppn;
			$total_pph += $invoice->besaran_pph;
			$total_invoice += $invoice->total_denganpph;
		...
		$sheet->setCellValue('E' . $row, $total_subtotal);
		$sheet->setCellValue('F' . $row, $total_diskon);
		$sheet->setCellValue('G' . $row, $total_ppn);
		$sheet->setCellValue('H' . $row, $total_pph);
		$sheet->setCellValue('I' . $row, $total_invoice);
		...
		$sheet->getStyle('A' . $header_row . ':M' . $row)->applyFromArray($styleArray);
		...
		// Set sheet title
		$sheet->setTitle('Report Invoice');

		// Generate filename berdasarkan filter
		$filename = 'Rekap_Invoice_';

		// Tambahkan nama customer jika dipilih
		if ($id_customer !== 'all') {
			$customer_name = '';
			...
			// Clean nama customer dari karakter spesial
			$customer_name = preg_replace('/[^A-Za-z0-9_\-]/', '_', $customer_name);
			$filename .= $customer_name . '_';
		} else {
			$filename .= 'Semua_Customer_';
		}

		// Tambahkan periode tanggal
		if (!empty($tanggal_dari) && !empty($tanggal_sampai)) {
			$filename .= date('Ymd', strtotime($tanggal_dari)) . '_sd_' . date('Ymd', strtotime($tanggal_sampai));
		} elseif (!empty($tanggal_dari)) {
			$filename .= 'dari_' . date('Ymd', strtotime($tanggal_dari));
		} elseif (!empty($tanggal_sampai)) {
			$filename .= 'sampai_' . date('Ymd', strtotime($tanggal_sampai));
		} else {
	...
	public function cetak_bukti_pembayaran_invoice($id)
	{
		// Get invoice data
		$data['invoice'] = $this->m_invoice->get_invoice_by_id($id);
		...
		// Get customer data
		$data['customer'] = $this->M_Customer->get_customer_by_id($data['invoice']->id_customer);

		// Get settings
		// $settings = $this->db->where_in('key', ['no_contact', 'email', 'alamat', 'nama_perusahaan', 'logo'])->get('setting')->result_array();
		// $data['settings_map'] = [];
		// foreach ($settings as $s) {
		// 	$data['settings_map'][$s['key']] = $s['value'];
		// }
